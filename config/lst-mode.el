;;; lst-mode.el --- mode for editing assembler code
...
;; inspired by an earlier lst-mode by Martin Neitzel.
...
;;   1) An lst-mode-set-comment-hook before the part of the initialization
;; depending on asm-comment-char, and
;;   2) an lst-mode-hook at the end of initialization.
...
(defvar lst-mode-abbrev-table nil
  "Abbrev table used while in Asm mode.")
(define-abbrev-table 'lst-mode-abbrev-table ())

(defvar lst-mode-map
  (let ((map (make-sparse-keymap)))
    ;; Note that the comment character isn't set up until lst-mode is called.
    (define-key map ":"		'asm-colon)
    (define-key map "\C-c;"	'comment-region)
    (define-key map "\C-j"	'newline-and-indent)
    (define-key map "\C-m"	'newline-and-indent)
    (define-key map [menu-bar lst-mode] (cons "Asm" (make-sparse-keymap)))
    (define-key map [menu-bar lst-mode comment-region]
      '(menu-item "Comment Region" comment-region
		  :help "Comment or uncomment each line in the region"))
    (define-key map [menu-bar lst-mode newline-and-indent]
      '(menu-item "Insert Newline and Indent" newline-and-indent
		  :help "Insert a newline, then indent according to major mode"))
    (define-key map [menu-bar lst-mode asm-colon]
...
(defconst asm-font-lock-keywords
  (append
   '(
     ;; ( "^\\s-+[0-9]+\\s-+[0-9A-F]+\\s-+[0-9A-F]+" . font-lock-type-face)
...
;;;###autoload
(define-derived-mode lst-mode prog-mode "Assembler"
...
Alternatively, you may set this variable in `lst-mode-set-comment-hook',
which is called near the beginning of mode initialization.

Turning on Asm mode runs the hook `lst-mode-hook' at the end of initialization.

Special commands:
\\{lst-mode-map}"
  (setq local-abbrev-table lst-mode-abbrev-table)
  ...
  (run-hooks 'lst-mode-set-comment-hook)
  ;; Make our own local child of lst-mode-map
  ;; so we can define our own comment character.
  (use-local-map (nconc (make-sparse-keymap) lst-mode-map))
  (local-set-key (vector asm-comment-char) 'asm-comment)
  (set-syntax-table (make-syntax-table lst-mode-syntax-table))
